...
import './search-followers.scss'

export class SearchFollowers extends Component {
    state = {
        showInformationFollower : false,
        followerSearchError: true
    }
    ...
    handleSubmitFollower = (e) => {
        ...
        if(this.state.followerInput === followerFilter) {
            ...
            setTimeout(this.setState ({ showInformationFollower : true}),1000)
            this.setState({ followerSearchError: true })
        } else if (this.state.followerInput !== null && this.state.followerInput !== followerFilter) {
            return (
                this.setState({ followerSearchError: false })
            )
        }
    }
    showInformationAboutFollower = () => {
        this.props.informationAboutFollowerFetchData(`https://api.github.com/users/${this.state.followerInput}`)
    }
    ...
    render() {
        return (
            <>
                <div className='search search__followers'>
                    ...
                    <div className={`Error__message ${this.state.followerSearchError ? 'followerSearchError' : null}`}>It's not a follower of this user</div>
...
export default connect(mapStateToProps,mapDispatchToProps)(SearchFollowers)
