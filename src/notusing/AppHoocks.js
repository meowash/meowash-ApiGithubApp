...
function App() {
  ...
  const [nameOfFollower, setNameOfFollower] = useState('')
  ...
  useEffect(() => {
    fetch('https://api.github.com/users/meowash')
    .then(res => res.json())
    .then(user => {
      setUser(user);
    })
    ...
    <div className="App">
      <div className="search" onSubmit={handleSubmit}>
        <div className="search__intro">
          <input type="text" onChange={handleSearch} onKeyPress={handleEnter} placeholder='Find github profile' className="search__input"/>
      ...
      { error ? (<h1> {error}</h1>
        ...
        <div className="cart">
          <div className="cart__user-title">
            <div className="cart__name">Login of user: {userName}</div>
            <img className='cart__img' src={avatar} alt=""/>
            <button className="btn__followers" >Followers : {followers}</button>
          </div>
